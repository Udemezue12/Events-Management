...
class StateRoutes:
    ...
    @router.get("/get/states/venues", dependencies=[rate_limit])
    @safe_handler
    async def get_states_with_venues(
        self,
        page: int = 1, 
        per_page: int = 20,
        db: AsyncSession = Depends(get_db_async),
        ...
        return await StateService(db).get_all_states_with_venues(per_page=per_page, page=page)
    ...
    async def get_states(
        ...
        return await StateService(db).get_all_states_without_pagination()
