...
class StateRepo:
    ...
    async def get_all(self, page: int = 1, per_page: int = 20):
        result = await self.db.execute(
            select(State)
            .options(selectinload(State.venues))
            .order_by(State.name)
    ...
    async def get_all_states(self, page: int = 1, per_page: int = 20) -> list[State]:
        ...
        return result.scalars().all()
    async def get_all_states_without_pagination(self,) -> list[State]:
        result = await self.db.execute(
            select(State)
            .order_by(State.name)
    ...
    async def get_one(self, name: str)-> Optional[State]:
        result = await self.db.execute(
            select(State).where(State.name == name).options(selectinload(State.venues))
        )
    ...
    async def get_all_with_venues(self, page: int = 1, per_page: int = 20):
        result = await self.db.execute(
            select(State)
            .options(selectinload(State.venues))
            .order_by(State.name)
